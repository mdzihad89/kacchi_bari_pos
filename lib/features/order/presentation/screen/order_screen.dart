    ...
    return PagedDataTableTheme(
      ...
      child: PagedDataTable<String, BranchOrder>(
        ...
        fetcher: (pageSize, sortModel, filterModel, pageToken) async {
          ...
          dynamic orderDateFrom= filterModel["orderDateRange"]!=null? (filterModel["orderDateRange"] as DateTimeRange).start.toIso8601String():null;
          dynamic orderDateTo= filterModel["orderDateRange"]!=null? (filterModel["orderDateRange"] as DateTimeRange).end.toIso8601String():null;
          OrderFilter orderFilter=OrderFilter(
              ...
              orderDate: filterModel["orderDate"] != null ? (filterModel["orderDate"] as DateTime).toIso8601String() : null,
          ...
          return orderResult.fold(
                (failure) {
                  ElegantNotification.error(
                    ...
                    description:  Text(failure.message, style: TextStyle(color: Colors.black), maxLines: 1),
                    width: 300,
                    height: 100,
                  ).show(context);
                ...
                (orderResponse) => (orderResponse.orders, orderResponse.nextPageToken),
          );
        ...
        filters: [
          ...
          DropdownTableFilter<String>(
            items: paymentMode.map((e) => DropdownMenuItem(value: e, child: Text(e,style: Theme.of(context).textTheme.bodyLarge,))).toList(growable: false),
          ...
          DropdownTableFilter<String>(
            items: paymentStatus.map((e) => DropdownMenuItem(value: e, child: Text(e,style: Theme.of(context).textTheme.bodyLarge,))).toList(growable: false),
            chipFormatter: (value) =>
        ...
        columns: [
          ...
          TableColumn(
            ...
            cellBuilder: (context, item, index) => Text(DateFormat('dd-MM-yyyy').format(DateTime.parse(item.orderDate))),
            size: const FixedColumnSize(120),
          ),
          TableColumn(
            title: const Text("SubTotal"),
            cellBuilder: (context, item, index) => Text("${item.subtotalAmount} Tk"),
          ...
          TableColumn(
            ...
            cellBuilder: (context, item, index) => Text("${item.deliveryFee} Tk"),
            size: const FixedColumnSize(100),
          ),
          TableColumn(
          ...
          TableColumn(
            ...
            cellBuilder: (context, item, index) =>  Text("${item.paidAmount} Tk"),
            size: FixedColumnSize(100),
          ),
          TableColumn(
    ...
    ? Colors.red
        : Colors.white),),
                              ...
                              children: [
                                ...
                                TextButton(
                                    onPressed: () async{
                                      ...
                                      OrderRepositoryImpl orderRepository = instance<OrderRepositoryImpl>();
                                      final orderResult = await orderRepository.updatePendingOrderOnline(orderStatusUpdateModel);
                                      setState(() {
                                        isLoading=false;
                                      });
                                      orderResult.fold(
                      ...
                      content: SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final singleItem = CartItem.fromCart(item.cartItems[index]);
                                              ...
                                              if (_formKey.currentState!
                                                ...
                                                if (changeAmount! < 0) {
                                                  ...
                                                  final dineInPaymentModelOnline = DineInPaymentModelOnline(
                                                    ...
                                                    discountAmount: _discountAmountController.text.isEmpty?0:int.tryParse(_discountAmountController.text)!,
                                                    paidAmount: int.tryParse(_paidAmountController.text)!,
                                                    netPayableAmount:  item.subtotalAmount-(_discountAmountController.text.isEmpty?0:int.tryParse(_discountAmountController.text)!),
                                                  ...
                                                  OrderRepositoryImpl orderRepository = instance<OrderRepositoryImpl>();
                                                  final orderResult = await orderRepository.updatePendingOrderOnline(dineInPaymentModelOnline);
                                                  setState(() {
                                                    isLoading=false;
                                                  });
                                                  orderResult.fold(
                                            ...
                                            onPressed: () async{
                                              if (_formKey.currentState!.validate()) {
                                                if (changeAmount! < 0) {
                                                  ...
                                                  OrderRepositoryImpl orderRepository = instance<OrderRepositoryImpl>();
                                                  final orderResult = await orderRepository.updatePendingOrderOnline(deliveryPaymentModelOnline );
                                                  setState(() {
                                                    isLoading=false;
                                                  });
                                                  orderResult.fold(
                                            ...
                                            onPressed: () async{
                                              if (_formKey.currentState!.validate()) {
                                                if (changeAmount! < 0) {
                                                  ...
                                                  OrderRepositoryImpl orderRepository = instance<OrderRepositoryImpl>();
                                                  final orderResult = await orderRepository.updatePendingOrderOnline(preOrderPaymentModelOnline);
                                                  setState(() {
                                                    isLoading=false;
                                                  });
                                                  orderResult.fold(
          ...
          TableColumn(
            ...
            cellBuilder: (context, item, index) => Text(item.customerPhoneNumber),
            size: const FixedColumnSize(150),
            onTap: (item, index) {
